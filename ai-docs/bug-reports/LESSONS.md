...
| 2026-07-26 | review 一個 migration 的**內容**改得對不對之前,先問「這個檔案可不可以改」。我把 PR #15 的 V9 取捨(`CONCURRENTLY` 在 Flyway 下死鎖)驗到 `pg_stat_activity` 層級,卻沒發現 V9 早已隨 PR #13 合併——改它直接違反 `flyway-convention.md:33`。判斷檔案的**可變性**要先於判斷內容的正確性 | CRC32 實測:原始 V9 `1874006957` → 改後 `2297068974`,已套用環境會 checksum mismatch 啟動失敗。抓到的是並行 session 的 `04-RESEARCH.md:2214`,不是我 | new |

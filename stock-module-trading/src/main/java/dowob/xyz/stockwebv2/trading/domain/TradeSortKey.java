...
public enum TradeSortKey {
    ...
    public static TradeSortKey fromApiValue(String value) {
        ...
        for (TradeSortKey key : values()) {
            if (key.apiValue.toUpperCase(Locale.ROOT).equals(normalized)) {
                return key;
            }
        }
        throw new BusinessException(ErrorCode.VALIDATION_FAILED, "sort must be one of executedAt, total, quantity");
    }
}
